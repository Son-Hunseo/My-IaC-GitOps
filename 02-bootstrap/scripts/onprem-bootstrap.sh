...
#   2) 클러스터 설치     : kubespray playbooks/cluster.yml
#   3) Argo CD add-on   : 02-bootstrap 자체 플레이북
#
...
echo "==> 2/3 kubespray 클러스터 설치 (${KUBESPRAY_VERSION})"
(
  cd "${KUBESPRAY_DIR}"
...
echo "==> 3/3 Argo CD add-on 설치"
ANSIBLE_CONFIG="${ONPREM_CFG}" ansible-playbook \
  -i "${INVENTORY}" \
  "${BOOTSTRAP_DIR}/onprem-install-argocd.yml" "$@"
